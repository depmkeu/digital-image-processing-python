...
for y in range(1, h-1):
    for x in range(1, w-1):
        ...
        # Gy
        Gy = (-p[0][0] -2*p[0][1] - p[0][2]
              + p[2][0] +2*p[2][1] + p[2][2])

        v_sum = abs(Gx) + abs(Gy)
        v_max = max(abs(Gx), abs(Gy))
        v_avg = (abs(Gx) + abs(Gy)) // 2
        v_mag = int(math.sqrt(Gx*Gx + Gy*Gy))
...
Image.fromarray(K0_sum).save("edge_results/sobel_sum.png")
Image.fromarray(K0_max).save("edge_results/sobel_max.png")
Image.fromarray(K0_avg).save("edge_results/sobel_avg.png")
Image.fromarray(K0_mag).save("edge_results/sobel_mag.png")

print("Hasil Sobel disimpan di 'edge_results' (sobel_*.png)\n")
...
plt.subplot(2,3,3); plt.imshow(K0_sum, cmap="gray"); plt.title("Sobel: SUM"); plt.axis("off")
plt.subplot(2,3,4); plt.imshow(K0_max, cmap="gray"); plt.title("Sobel: MAX"); plt.axis("off")
plt.subplot(2,3,5); plt.imshow(K0_avg, cmap="gray"); plt.title("Sobel: AVERAGE"); plt.axis("off")
plt.subplot(2,3,6); plt.imshow(K0_mag, cmap="gray"); plt.title("Sobel: MAGNITUDE"); plt.axis("off")
