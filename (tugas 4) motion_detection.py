...
# === CEK DAN SAMAKAN UKURAN (OTOMATIS) ===
if A.size != B.size:
...
# === FUNGSI TAMBAHAN UNTUK TAMPILKAN PROPERTI ===
...
wa = 1
wb = -1
threshold = 30  # opsional: batas minimum perubahan dianggap gerakan

# === PROSES DETEKSI GERAK MANUAL ===
C = Image.new("RGB", (w, h))
...
for y in range(h):
    for x in range(w):
        ...
        # Rumus: C(x,y) = |A - B|
        r = abs(int((wa * rA) + (wb * rB)))
        g = abs(int((wa * gA) + (wb * gB)))
        b = abs(int((wa * bA) + (wb * bB)))

        # Threshold opsional untuk menonjolkan perubahan besar (gerakan nyata)
        r = 255 if r > threshold else 0
        g = 255 if g > threshold else 0
        b = 255 if b > threshold else 0

        # Batasi agar tetap dalam 0–255
        r = min(max(r, 0), 255)
        g = min(max(g, 0), 255)
        b = min(max(b, 0), 255)
...
os.makedirs("motion_result", exist_ok=True)
result_path = "motion_result/motion_detection_result.png"
C.save(result_path)

print(f"\nDeteksi gerakan selesai! Hasil disimpan di: {result_path}")

# === TAMPILKAN SEMUA CITRA ===
plt.figure("DETEKSI GERAKAN", figsize=(12, 5))
...
plt.subplot(1, 3, 3)
plt.imshow(C)
plt.title("Hasil Deteksi Gerakan (|A-B|)")
plt.axis("off")
