...
##  [ 0,  0,  0]
##  [ 1,  1,  1]
## Kombinasi: K0 = |K1| + |K2|, max(|K1|,|K2|), average, magnitude
...
# Konversi grayscale untuk operasi
...
# Proses Prewitt (sum of products)
for y in range(1, h-1):
    for x in range(1, w-1):
        # ambil window 3x3
        p = [[pixels[x+i, y+j] for i in range(-1,2)] for j in range(-1,2)]

        # K1 horizontal (prewitt)
        K1 = (-p[0][0] + p[0][2]
             -p[1][0] + p[1][2]
             -p[2][0] + p[2][2])

        # K2 vertical (prewitt)
        K2 = (-p[0][0] - p[0][1] - p[0][2]
              + p[2][0] + p[2][1] + p[2][2])
...
Image.fromarray(K0_sum).save("edge_results/prewitt_sum.png")
Image.fromarray(K0_max).save("edge_results/prewitt_max.png")
Image.fromarray(K0_avg).save("edge_results/prewitt_avg.png")
Image.fromarray(K0_mag).save("edge_results/prewitt_mag.png")

print("Hasil Prewitt disimpan di 'edge_results' (prewitt_*.png)\n")
...
plt.subplot(2,3,3); plt.imshow(K0_sum, cmap="gray"); plt.title("Prewitt: SUM"); plt.axis("off")
plt.subplot(2,3,4); plt.imshow(K0_max, cmap="gray"); plt.title("Prewitt: MAX"); plt.axis("off")
plt.subplot(2,3,5); plt.imshow(K0_avg, cmap="gray"); plt.title("Prewitt: AVERAGE"); plt.axis("off")
plt.subplot(2,3,6); plt.imshow(K0_mag, cmap="gray"); plt.title("Prewitt: MAGNITUDE"); plt.axis("off")
